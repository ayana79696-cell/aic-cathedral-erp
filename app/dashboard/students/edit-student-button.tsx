...
export default function EditStudentButton({student,classes,streams,routes}:{student:any;classes:any[];streams:any[];routes:Route[]}){
 ...
 const[f,setF]=useState<any>({...student,uses_bus:Boolean(student.uses_bus)})
 const set=(k:string,v:any)=>setF((x:any)=>({...x,[k]:v}))
 const cls=streams.filter(x=>x.class_id===f.class_id)
 const save=async(e:any)=>{e.preventDefault();setBusy(true);setError('');try{const fd=new FormData();Object.entries(f).forEach(([k,v])=>{if(k!=='id'&&k!=='stream_id'&&v!==undefined&&v!==null)fd.set(k,String(v))});const selected=streams.find(x=>x.id===f.stream_id);fd.set('stream_name',selected?.name||'');fd.set('student_id',student.id);fd.set('uses_bus',String(Boolean(f.uses_bus)));await updateStudent(fd);setOpen(false);window.location.reload()}catch(x:any){setError(x?.message||'Could not update learner.')}finally{setBusy(false)}}
 const selectedRoute=routes.find(r=>r.route_name===f.bus_route)
 return <>
  ...
  {open&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.45)',zIndex:50,overflow:'auto',padding:24}}>
   <form onSubmit={save} className="prototype-panel" style={{maxWidth:900,margin:'30px auto',padding:20}}>
    ...
    <div className="prototype-form-grid">
     ...
     {f.uses_bus&&<>
      <div><label>Bus route</label><select value={f.bus_route??''} onChange={e=>{set('bus_route',e.target.value);const r=routes.find(x=>x.route_name===e.target.value);set('bus_pickup_point',r?.pickup_points||'')}}><option value="">Select saved route</option>{routes.map(r=><option key={r.id} value={r.route_name}>{r.route_name}{r.vehicle_no?` — ${r.vehicle_no}`:''}</option>)}</select>{selectedRoute?.pickup_points&&<small className="muted">Pickup points: {selectedRoute.pickup_points}</small>}</div>

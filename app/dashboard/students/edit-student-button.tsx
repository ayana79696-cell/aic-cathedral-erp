  ...
  {open&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.45)',zIndex:50,overflow:'auto',padding:24}}>
   <form onSubmit={save} className="prototype-panel" style={{maxWidth:900,margin:'30px auto',padding:20}}>
    ...
    <div className="prototype-form-grid">
     ...
     {f.uses_bus&&<>
      <div><label>Bus route</label><select value={f.bus_route??''} onChange={e=>{const value=e.target.value;const r=routes.find(x=>x.route_name===value);set('bus_route',value);set('bus_route_id',r?.id||'');set('bus_pickup_point',r?.pickup_points||'')}}><option value="">Select saved route</option>{routes.map(r=><option key={r.id} value={r.route_name}>{r.route_name}{r.vehicle_no?` — ${r.vehicle_no}`:''}</option>)}</select>{selectedRoute?.pickup_points&&<small className="muted">Pickup points: {selectedRoute.pickup_points}</small>}</div>

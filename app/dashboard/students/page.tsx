...
export default async function Students(){
 ...
 return <main className="main">
  ...
  <section className="card" style={{marginTop:16}}><h2>Student register</h2><p className="muted">The portal code is safe to give to the child's parent after the parent account is activated.</p><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Portal Code','Admission','Student','Grade','Stream','Blood','Medical / needs','Status',...(isAdmin?['Actions']:[])].map(h=><th key={h} style={{textAlign:'left',padding:10}}>{h}</th>)}</tr></thead><tbody>{students?.length?students.map(x=><tr key={x.id}><td style={{padding:10,fontWeight:700,color:'#0757a6',whiteSpace:'nowrap'}}>{x.portal_code||'Generating…'}</td><td style={{padding:10}}>{x.admission_number}</td><td style={{padding:10}}>{studentName(x)}</td><td style={{padding:10}}>{classMap.get(x.class_id)||'—'}</td><td style={{padding:10}}>{streamMap.get(x.stream_id)||'—'}</td><td style={{padding:10}}>{x.blood_group||'—'}</td><td style={{padding:10}}>{x.medical_conditions||'None recorded'}</td><td style={{padding:10}}>{x.status}</td>{isAdmin&&<td style={{padding:10}}><DeleteStudentButton id={x.id} name={studentName(x)}/></td>}</tr>):<tr><td colSpan={isAdmin?9:8} style={{padding:35,textAlign:'center'}} className="muted">No students yet.</td></tr>}</tbody></table></div></section>
 </main>
}

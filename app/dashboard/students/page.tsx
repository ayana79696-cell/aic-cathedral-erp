...
export default async function Students(){
 ...
 const [{data:students},{data:classes},{data:streams}]=await Promise.all([
  s.from('students').select('id,admission_number,first_name,middle_name,last_name,gender,date_of_birth,class_id,stream_id,blood_group,medical_conditions,disability_special_needs,status').order('last_name'),
 ...
 return <main className="main">
  <header className="top"><div><h1>Students</h1><p className="muted">Complete learner admission, guardian, class/stream and health records.</p></div></header>
  <section className="card">
   <h2>New student admission</h2>
   <p className="muted">Capture the learner's school, guardian, emergency and medical information in one record.</p>
  ...
  <section className="card" style={{marginTop:16}}><h2>Student register</h2><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Admission','Student','Grade','Stream','Blood','Medical / needs','Status'].map(h=><th key={h} style={{textAlign:'left',padding:10}}>{h}</th>)}</tr></thead><tbody>{students?.length?students.map(x=><tr key={x.id}><td style={{padding:10}}>{x.admission_number}</td><td style={{padding:10}}>{x.first_name} {x.middle_name||''} {x.last_name}</td><td style={{padding:10}}>{classMap.get(x.class_id)||'—'}</td><td style={{padding:10}}>{streamMap.get(x.stream_id)||'—'}</td><td style={{padding:10}}>{x.blood_group||'—'}</td><td style={{padding:10}}>{x.medical_conditions||x.disability_special_needs||'None recorded'}</td><td style={{padding:10}}>{x.status}</td></tr>):<tr><td colSpan={7} style={{padding:35,textAlign:'center'}} className="muted">No students yet.</td></tr>}</tbody></table></div></section>
 </main>
}
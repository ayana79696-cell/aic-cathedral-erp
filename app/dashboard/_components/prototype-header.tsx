...
const titles: Record<string, string> = {
  ...
  '/dashboard/academic': 'Academic Setup',
  '/dashboard/exams': 'Exams & CBC',
  ...
  '/dashboard/analytics': 'School Intelligence',
...
export default function PrototypeHeader({ items, fullName, role, email }: { items: Item[]; fullName: string; role: string; email?: string | null }) {
  ...
  return <>
    <header className="prototype-mobile-header no-print">
      ...
      <strong>{title}</strong>
      <span className="prototype-header-spacer" />
      <span className="prototype-demo-pill">● DEMO MODE – SAMPLE DATA</span>
      <div className="prototype-user-chip">
        <span className="prototype-avatar">SA</span>
        <span><b>{roleLabel[role] || role || 'User'}</b><small>Term 2, 2026</small></span>
      </div>
    </header>
    {open && <div className="prototype-drawer-backdrop no-print" onClick={() => setOpen(false)}>
      <nav className="prototype-drawer" onClick={e => e.stopPropagation()}>

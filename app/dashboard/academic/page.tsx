...
export default async function Academic(){
 const s=await createClient();const {data:classes}=await s.from('classes').select('id,name,level').eq('status','active').order('name');const classOptions=(classes||[]).map(c=>`${c.id} — ${c.name}${c.level?` (${c.level})`:''}`)
 ...
 <SimpleCrud table="streams" title="Streams — East / West" fields={[{name:'class_id',label:'Class ID',required:true,options:classOptions},{name:'name',label:'Stream name',required:true,options:['East','West','A','B','C']}]} />

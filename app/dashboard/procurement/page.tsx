...
import LpoManager from './lpo-manager'
...
function ProcurementSection(){return <PrototypePage title="Procurement" subtitle="Choose one procurement function at a time. Only the selected function is shown." tabs={[["#requisitions","Requisitions"],["#documents","LPOs & Job Cards"],["#suppliers","Suppliers"],["#approvals","Approvals"]]}>
 <section id="requisitions"><SimpleCrud table="procurement_requests" title="Requisitions" fields={[{name:'item_name',label:'Item / service',required:true},{name:'quantity',label:'Quantity',type:'number',required:true},{name:'estimated_cost',label:'Estimated cost',type:'number',required:true},{name:'status',label:'Approval status',options:['pending','approved','ordered','received','rejected']}]} roleHint="Approve a requisition first, then generate its LPO automatically from the LPO workspace."/></section>
 <section id="documents"><LpoManager/><div style={{marginTop:20}}><SimpleCrud table="procurement_documents" title="Job Cards, Invoices & Delivery Notes" fields={[{name:'document_type',label:'Document',options:['Job Card','Invoice','Delivery Note']},{name:'document_no',label:'Document No.'},{name:'amount',label:'Amount',type:'number'},{name:'status',label:'Status',options:['draft','generated','approved','received','paid']}]} /></div></section>

...
`}</style>
  ...
  <section className="card" style={{marginTop:16}}>
   ...
   <div className="prototype-table-wrap"><table className="prototype-table"><thead><tr><th>STUDENT</th><th>CLASS / STREAM</th><th>DATES</th><th>TYPE</th><th>REASON</th><th>STATUS</th><th>ACTION</th></tr></thead><tbody>{pending.map(r=>{const s=students.find(x=>x.id===r.student_id);return <tr key={r.id}><td><strong>{studentMap[r.student_id]||'Student'}</strong><br/><span className="muted">{s?.admission_number||''}</span></td><td>{s?.class_id?classes[s.class_id]||'—':'—'} / {s?.stream_id?streams[s.stream_id]||'—':'—'}</td><td>{r.start_date} → {r.end_date}{(r.start_time||r.end_time)&&<><br/><span className="muted">{r.start_time||'—'} → {r.end_time||'—'}</span></>}</td><td>{r.leave_type||r.suspension_type}</td><td>{r.reason}</td><td>{r.status}</td><td>{approverFor(tab)?<button className="btn" onClick={()=>{setDecision({kind:tab,id:r.id});setNote('');setNotifyParent(true);setParentMessage(tab==='leave'?'Dear Parent/Guardian, '+(studentMap[r.student_id]||'your child')+' has been granted approved leave from '+r.start_date+' to '+r.end_date+'. '+(r.leave_type==='medical'?'We wish your child a quick recovery. Please ensure they return to school on the stated date.':'Please take note of the approved leave period. Thank you for your cooperation. AIC Cathedral Comprehensive School.')+(r.start_time?' Start time: '+r.start_time+'. ':'')+(r.end_time?' Return/end time: '+r.end_time+'.':''):'Dear Parent/Guardian, '+(studentMap[r.student_id]||'your child')+' has been placed on an approved school suspension from '+r.start_date+' to '+r.end_date+'. Please contact the school/class teacher if you require clarification. Your cooperation is appreciated. AIC Cathedral Comprehensive School.'+(r.start_time?' Start time: '+r.start_time+'. ':'')+(r.end_time?' Return/end time: '+r.end_time+'.':''))}}>Review & Sign</button>:<span className="muted">Awaiting approver</span>}</td></tr>})}{pending.length===0&&<tr><td colSpan={7} className="prototype-empty">No pending requests.</td></tr>}</tbody></table></div>
  </section>

...
export default function StudentStatus(){
 ...
 const decide=async(kind:'leave'|'suspension',id:string,status:'approved'|'rejected')=>{
  ...
  setLoading(false);if(error)setMsg(error.message);else{setDecision(null);setNote('');setSignature('');setSignatureMode('draw');setMsg((kind==='leave'?'Leave':'Suspension')+' '+status+'.');await load()}
 }
 ...
 return <main className="main">
  ...
  {decision&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.45)',display:'grid',placeItems:'center',zIndex:1000,padding:20}}><div className="card" style={{maxWidth:700,width:'100%',maxHeight:'94vh',overflow:'auto'}}><h2>{decision.kind==='leave'?'Approve':'Review'} Student {decision.kind==='leave'?'Leave':'Suspension'}</h2><p className="muted">Class Teacher / Head Teacher signature. Draw on the box or type your name, exactly like the Teachers Leave workflow.</p><div style={{display:'flex',gap:8,margin:'12px 0'}}><button className={signatureMode==='draw'?'btn':'btn secondary'} onClick={()=>setSignatureMode('draw')}>Draw Signature</button><button className={signatureMode==='type'?'btn':'btn secondary'} onClick={()=>setSignatureMode('type')}>Type Name</button></div>{signatureMode==='draw'?<><canvas ref={el=>{setSignatureCanvas(el)}} style={{display:'block',width:'100%',height:140,border:'1px dashed #94a3b8',borderRadius:8,background:'#fff',touchAction:'none'}} onPointerDown={e=>{const canvas=e.currentTarget;canvas.setPointerCapture(e.pointerId);const r=canvas.getBoundingClientRect();const ctx=canvas.getContext('2d');ctx?.beginPath();ctx?.moveTo(e.clientX-r.left,e.clientY-r.top);(canvas as any)._drawing=true}} onPointerMove={e=>{const canvas=e.currentTarget;if(!(canvas as any)._drawing)return;const r=canvas.getBoundingClientRect();const ctx=canvas.getContext('2d');if(!ctx)return;ctx.lineWidth=2;ctx.lineCap='round';ctx.lineTo(e.clientX-r.left,e.clientY-r.top);ctx.stroke()}} onPointerUp={e=>{const canvas=e.currentTarget;(canvas as any)._drawing=false;setSignature(canvas.toDataURL('image/png'))}} onPointerCancel={e=>{const canvas=e.currentTarget;(canvas as any)._drawing=false;setSignature(canvas.toDataURL('image/png'))}}/><button className="btn secondary" style={{marginTop:7}} onClick={()=>{if(signatureCanvas){signatureCanvas.getContext('2d')?.clearRect(0,0,signatureCanvas.width,signatureCanvas.height)}setSignature('')}}>Clear</button></>:<input style={{width:'100%',height:55,fontSize:24,fontFamily:'cursive'}} placeholder="Enter your name as signature" value={signature.startsWith('data:image/')?'':signature} onChange={e=>setSignature(e.target.value)}/>}<textarea rows={4} placeholder="Decision note (optional)" value={note} onChange={e=>setNote(e.target.value)} style={{width:'100%',marginTop:12}}/><div style={{display:'flex',gap:10,marginTop:14}}><button className="btn" disabled={loading||!signature} onClick={()=>decide(decision.kind,decision.id,'approved')}>Approve & Sign</button><button className="btn secondary" disabled={loading||!signature} onClick={()=>decide(decision.kind,decision.id,'rejected')}>Reject & Sign</button><button className="btn secondary" onClick={()=>{setDecision(null);setSignature('');setNote('')}}>Cancel</button></div></div></div>}

...
const homeByRole: Record<string, string> = {
  ...
  hr: '/dashboard/hr',
  operations: '/dashboard/procurement',
...
export async function middleware(request: NextRequest) {
  ...
  if (path === '/login' && user) {
    const { data: p } = await s.from('profiles').select('role,must_change_password').eq('id', user.id).maybeSingle()
    if (p?.must_change_password) return NextResponse.redirect(new URL('/change-password', request.url))
  ...
  if (user && path.startsWith('/dashboard')) {
    ...
    if (path === '/dashboard' && p?.role && p.role !== 'super_admin') {
      return NextResponse.redirect(new URL(homeByRole[p.role] || '/dashboard', request.url))
    }

    const base = Object.keys(access).find(x => path === x || path.startsWith(x + '/'))
    if (base && p?.role !== 'super_admin' && !access[base]?.includes(p?.role || '')) {
      return NextResponse.redirect(new URL(homeByRole[p?.role || ''] || '/dashboard', request.url))
    }
  }
  return response
